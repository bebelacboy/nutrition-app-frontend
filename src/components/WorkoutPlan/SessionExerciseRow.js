...
import { selectExercise, setExerciseSet, setExerciseReps } from "../../slices/workoutPlanSlice";
...
export const SessionExerciseRow = ({ exercise, day, index }) => {
  ...
  return <div>
    <div className="flex gap-2 mx-auto pt-1 w-64 items-center">
      <label htmlFor="exerciseName" className="w-3/6 font-semibold">Exercise Name</label>      
      <label htmlFor="set" className="w-1/6 font-semibold">Set</label>      
      <label htmlFor="reps" className="w-1/6 font-semibold">Rep</label>      
    </div>
    ...
    <div className="flex gap-2 mx-auto pt-1 pb-2 w-64 items-center">
      <div className="w-3/6">
        ...
        <ul className={`${!showOptions && 'hidden'} max-h-48 overflow-y-scroll  absolute border-solid border border-black z-10`}>
          {showOptions && 
